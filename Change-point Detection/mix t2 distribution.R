df_PELT = matrix(,nrow=1000,ncol=200);df_RMosumMerged = matrix(,nrow=1000,ncol=20);df_RMosumPvalue = matrix(,nrow=1000,ncol=20);
df_PELT = matrix(,nrow=1000,ncol=200);df_RMosumMerged = matrix(,nrow=1000,ncol=20);df_RMosumPvalue = matrix(,nrow=1000,ncol=20);
df_WBS = matrix(,nrow=1000,ncol=200);df_cumseg = matrix(,nrow=1000,ncol=200);df_RANK = matrix(,nrow=1000,ncol=20);
df_ECP = matrix(,nrow=1000,ncol=200);df_NMCD = matrix(,nrow=1000,ncol=20);df_MosumMerged = matrix(,nrow=1000,ncol=20);df_MosumPvalue = matrix(,nrow=1000,ncol=20);
options (warn = - 1)
for(i in 1:1000){
  ...
  mub = multiscale.bottomUp(rank(x),G = c(10,25,50,60),eta = 2/3,var.est.method = 'mosum')$cpts
  y2=length(mub)
  ...
  mlp = multiscale.localPrune(rank(x), G = c(10,25,50,60),eta = 2/3,var.est.method = 'mosum')$cpts
  y3=length(mlp)
  ...
  y7=length(e.cp3o_delta(as.matrix(x),K =15,delta=30)$estimates)
  df_ECP[i,ncol(df_ECP)]=y7
  if (y7<1)
  {df_ECP[i,]=0}
  else{df_ECP[i,c(1:y7)]=e.cp3o_delta(as.matrix(x),K =15,delta=30)$estimates}
  write.csv(df_ECP,"D:/shicuo/ECP.csv")
  y8=length(cpts(cpt.np(x,penalty = 'SIC',minseglen = 50)))
  df_NMCD[i,ncol(df_NMCD)]=y8
  if (y8<1)
  {df_NMCD[i,]=0}
  else{df_NMCD[i,c(1:y8)]=cpts(cpt.np(x,penalty = 'SIC',minseglen = 50))}
  write.csv(df_NMCD,"D:/shicuo/NMCD.csv")
  mub = multiscale.bottomUp(x,G = c(10,25,50,60),eta = 2/3,var.est.method = 'mosum')$cpts
  y9=length(mub)
  ...
  mlp = multiscale.localPrune(x, G = c(10,25,50,60),eta = 2/3,var.est.method = 'mosum')$cpts
  y10=length(mlp)
...
#df_PELT
df_PELT1 = t(df_PELT)
True_changpoints = c(11,21,41,61,101,121,161,201,251,301,361,421,491)
