...
df_WBS = matrix(,nrow=1000,ncol=20);df_cumseg = matrix(,nrow=1000,ncol=200);df_RANK = matrix(,nrow=1000,ncol=20);
...
options (warn = - 1)
for(i in 1:1000){
  td1 <- testData(model = "stairs10",seed = i+3000)
  x <- td1$x
  y1=length(PELT(as.matrix(x), pen = 0.1, cost_func = "norm.mean", shape = 1, minseglen = 8)$cpts)
  df_PELT[i,ncol(df_PELT)]=y1
  if (y1<1)
  {df_PELT[i,]=0}
  else{df_PELT[i,c(1:y1)] = PELT(as.matrix(x), pen = 0.1, cost_func = "norm.mean", shape = 1, minseglen = 8)$cpts}#PELT
  write.csv(df_PELT,"D:/shicuo/PELT.csv")
  mub = multiscale.bottomUp(rank(x),G = c(8,10,20,30,50),eta = 2/3,var.est.method = 'mosum')$cpts
  y2=length(mub)
  ...
  mlp = multiscale.localPrune(rank(x), G = c(8,10,20,30,50),eta = 2/3,var.est.method = 'mosum')$cpts
  y3=length(mlp)
  ...
  cumseg <- jumpoints(rank(x),output="2")
  ...
  mub = multiscale.bottomUp(x,G = c(8,10,20,30,50),eta = 2/3,var.est.method = 'mosum')$cpts
  y9=length(mub)
  ...
  mlp = multiscale.localPrune(x, G = c(8,10,20,30,50),eta = 2/3,var.est.method = 'mosum')$cpts
  y10=length(mlp)
...
for(i in 1:1000){
  td1 <- testData(model = "stairs10",seed = i+3000)
  x <- td1$x
  y7=length(e.cp3o_delta(as.matrix(x),K =15,delta=9)$estimates)
  df_ECP[i,ncol(df_ECP)]=y7
  if (y7<1)
  {df_ECP[i,]=0}
  else{df_ECP[i,c(1:y7)]=e.cp3o_delta(as.matrix(x),K =15,delta=9)$estimates}
  write.csv(df_ECP,"D:/shicuo/ECP.csv")
}
table(df_PELT[,ncol(df_PELT)] - 14)/1000
table(df_RMosumMerged[,ncol(df_RMosumMerged)] - 14)/1000
table(df_RMosumPvalue[,ncol(df_RMosumPvalue)] - 14)/1000
table(df_MosumMerged[,ncol(df_MosumMerged)] - 14)/1000
table(df_MosumPvalue[,ncol(df_MosumPvalue)] - 14)/1000
table(df_WBS[,ncol(df_WBS)] - 14)/1000
table(df_cumseg[,ncol(df_cumseg)] - 14)/1000
table(df_RANK[,ncol(df_RANK)] - 14)/1000
table(df_ECP[,ncol(df_ECP)] - 14)/1000
table(df_NMCD[,ncol(df_NMCD)] - 14)/1000
